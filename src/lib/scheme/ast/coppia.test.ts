...
test('Coppia - primo e resto', () => {
	for (const car of [
		...
		Atomo.fromString('1'),
		Atomo.fromString('a'),
		new Coppia(Atomo.fromString('3'), null),
		new Coppia(Atomo.fromString('3'), Atomo.fromString('4'))
	]) {
		for (const cdr of [
			null,
			Atomo.fromString('""'),
...
test('Coppia - toString', () => {
	const testCases: [Coppia, string][] = [
		...
		[new Coppia(Atomo.fromString('"a"'), Atomo.fromString('"b"')), '("a" . "b")'],
		[
			new Coppia(new Coppia(Atomo.fromString('1'), Atomo.fromString('2')), Atomo.fromString('3')),
			'((1 . 2) . 3)'
		],
		[
			new Coppia(Atomo.fromString('1'), new Coppia(Atomo.fromString('2'), Atomo.fromString('3'))),
			'(1 . (2 . 3))'
		],
