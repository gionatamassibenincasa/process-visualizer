...
function parseArgs(argv: string[]): CLIOptions {
	if (argv.length === 0) {
		throw new Error(
			'Uso: stepper-cli <file-programma.scm> [--max-passi N] [--json] [--ambiente ID] [--interactive|-i]'
		);
	}
		...
		if (arg === '--ambiente') {
			...
			try {
				...
				if (error instanceof AmbienteNonTrovatoError) {
					throw new Error(`Ambiente da CLI non riconosciuto: "${idAmbiente}"`, { cause: error });
				}
				throw error;
			}
...
export async function eseguiStepperInterattivo(options: CLIOptions): Promise<void> {
	...
	for (const passo of stepper.passiStream(sorgente, options.maxPassi)) {
		...
		if (!passo.èTerminato) {
			await attendiPressioneInvio(
				'Premi [INVIO] per eseguire il prossimo passo (o Ctrl+C per uscire)... '
			);
		}
	}
